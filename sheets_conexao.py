...
df = df.rename(columns={"JANELA DE EXPEDIÇÃO": "data_exp", "STATUS TU": "status_tu", "CONF. PUXADA": "data_ats", "QTDE DE PEDIDOS": "qtde_pedidos", "TRANSP. + TP": "transportador", "TU(s)": "TU", "HORÁRIO": "horario", "CHECKOUT": "data_checkout"})
df = df[df["data_exp"] != ""].reset_index(drop=True)
print(f"Linhas apos remover vazias: {len(df)}")
...
df_envio = df[["data_exp", "limite_ats", "data_ats", "data_checkout","sla_ats","sla_expedicao","status_tu", "qtde_pedidos", "transportador", "TU", "horario", "chave",]].copy()
for col in ["data_exp", "limite_ats", "data_ats", "data_checkout"]:
    df_envio[col] = df_envio[col].dt.strftime("%d/%m/%Y %H:%M:%S").fillna("")
df_envio = df_envio.fillna("")
